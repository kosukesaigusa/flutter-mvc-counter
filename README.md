...
このリポジトリは近日公開予定の記事のサンプルディレクトリです。
...
View と Controller をあわせて UI（ユーザーインターフェース）といいます。その名の通り、ユーザーとアプリケーションの境界、両者とつなぐものです。
...
なお、以下の解釈や実装方針は一定の一貫性や納得感を伴うものではありますが、唯一の最善の正解というわけではないことには注意してください。また、以下では説明のしやすさの観点から、以下では M, V, C の順番を入れ替えています。
...
Flutter アプリにおいて、格闘ゲームのコントローラに対応する、ユーザーが触って操作する対象は、たとえば `ElevatedButton` などのボタンや `SelectableText` などのウィジェット (`StatefulWidget`) が代表的です。
...
- 「カウント」に数字が表示されている
- 「カウントアップ」ボタンを押すと、「カウント」に数字に 1 が加算される
- 「リストに追加」ボタンを押すと、現在のカウントの数字をリストに追加する
- 「合計値」に、現在のリストの合計値が表示されている
...
Dart (Flutter) におけるユニットテストの書き方の詳細はここでは説明しませんが、`ChangeNotifier` にしか依存していない `Counter` モデルは、次のように用意にユニットテストを書くことができ、その振る舞いを説明したり振る舞いの正しさを検査したりすることができます。

また、例えばカウントの値を、サーバサイドで実装した API と通信して送信・永続化するような機能を追加する場合には、`Counter` クラスのコンストラクタでインジェクションすることで、ユニットテストではそれをモックに置き換えて、テストを記述することがも容易です。
...
コンストラクタで `Counter` モデルのインスタンスを渡す方法はシンプルで、コントローラのテストを書きたくなった場合にもモデルを用意にモックに置き換えることができるので良いでしょう。
...
どのモジュールが何に依存して良くて、反対に依存するべきでないかを明確化し、どのように依存するのか具体例を示すことで、テスト容易性、依存性の注入、PDS (Presentation Domain Separation) などの概念に関しても、意識したり学んだりするきっかけになるとも思います。
